...
ess_error_t  ess_backend_uart_close( void ){
  uart_driver_delete(UART_NUM_1);
  return ESS_OK;
}
ess_error_t  ess_backend_uart_pause( void ){
  g_uart_paused = 1;
  return ESS_OK;
}
ess_error_t     ess_backend_uart_write( void *buffer, unsigned int buf_size,  unsigned int* wrote ){
  if(g_uart_paused) return ESS_ERROR_PAUSED;

  *wrote = uart_write_bytes(UART_NUM_1, (const char *) buffer, buf_size);
  return ESS_OK;
}
ess_error_t     ess_backend_uart_read( void *buffer, unsigned int buf_size, unsigned int* readed  ){
  if(g_uart_paused) return ESS_ERROR_PAUSED;
...
ess_error_t  ess_backend_uart_flush( void ){
  uart_flush(UART_NUM_1);
  return ESS_OK;
}
ess_error_t  ess_backend_uart_resume( void ){
  g_uart_paused = false;
  return ESS_OK;
}
ess_error_t  ess_backend_uart_set_sample_format(const ess_format_t format) {
  const char* buffer =  ess_format_to_string(format);
  uart_write_bytes(UART_NUM_1, (const char *) buffer, strlen(buffer));
